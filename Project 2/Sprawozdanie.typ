...
=== Krzysztof Nasuta, s193328 #linebreak() #datetime.today().display("[day]-[month]-[year]")
#set align(left)
#set heading(numbering: "1.")

= Wstęp

W niniejszym sprawozdaniu przedstawiono wyniki analizy trzech metod rozwiązywania macierzowych równań linowych. Do testowanych metod należa dwie metody iteracyjne: Jacobiego oraz Gaussa-Seidela oraz jedna metoda bezpośrednia: faktoryzacja LU. Kod źródłowy został napisany w języku Python, a do wizualizacji wyników wykorzystano bibliotekę Matplotlib. Nie wykorzystano żadnych zewnętrznych bibliotek do rozwiązywania układów równań liniowych. Do przechowywania macierzy napisano własną klasę `Matrix`, która ułatwia późniejsze operacje na macierzach. 

Powyżej wymionione metody zostały przetestowane na kilku układach równań i rozmiarów macierzy. Wyniki zostały przedstawione w postaci wykresów, które pokazują zbieżność metod iteracyjnych oraz czas wykonania dla każdej z metod.
...
Pierwszą z badanych metod iteracyjnych jest motoda Jacobiego. Zaimplementowana wersją wykorzystuje rekurencyjny wzór operujący na poszczególnych elementach wektora $x$. Wzór ten wygląda następująco:
$
...
Oczekawana normaa oraz limit nie uległy zmianie.
...
Dla metod iteracyjnych tempo wzrostu można oszacować jako kwadratowe względem rozmiarem macierzy. Dla metody faktoryzacji LU wzrost czasu był znacznie szybszy. Szacować go można jako co najmniej sześcienny względem rozmiaru macierzy. 
